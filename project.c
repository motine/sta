...
void draw() {
  ...
  if ((brick_pos_x == 0) && (ball_x - BRICK_OFFSET_X < 0)) { // workaround: usually if the ball is just left of a brick, brick_pos_x would round down and hence point to the left brick. In the case of the brick just left of the left-most, this behavior changes. The calculation will round up (e.g. -1/2 == 0). This causes brick_pos_x to be higher than expected and the brick right of the current position is removed even though it was not hit (yet).
    brick_pos_x = -1;
  }
  if ((brick_pos_y == 0) && (ball_y - BRICK_OFFSET_Y < 0)) {
    brick_pos_y = -1;
  }
  if ((brick_pos_x >= 0) && (brick_pos_x < BRICK_COUNT_X) &&
      (brick_pos_y >= 0) && (brick_pos_y < BRICK_COUNT_Y)) { // do we have a valid brick index?
